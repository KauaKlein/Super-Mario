export default function Movimentacao(scene) {
  ...
  player.isPulando = cursors.up.isDown && player.body.touching.down;
  ...
  if (player.isAgachando) {
    ...
    const direcao = player.isOlhandoFrente ? "olhandoFrente" : "olhandoCosta";
  ...
  if (player.isSubindo) {
    const direcao = player.isOlhandoFrente ? "pulandoFrente" : "pulandoCosta";
    if (player.anims.currentAnim?.key !== direcao) {
      player.anims.play(direcao);
    }
    return;
  ...
  if (player.isDescendo) {
    const direcao = player.isOlhandoFrente ? "caindoFrente" : "caindoCosta";
    if (player.anims.currentAnim?.key !== direcao) {
      player.anims.play(direcao);
    }
    return;
  ...
  if (player.body.velocity.x !== 0) {
    const anim = player.isOlhandoFrente ? "andandoFrente" : "andandoCosta";
    if (player.anims.currentAnim?.key !== anim) {
      player.anims.play(anim);
    }
  } else {
    const anim = player.isOlhandoFrente ? "paradoFrente" : "paradoCosta";
    if (player.anims.currentAnim?.key !== anim) {
      player.anims.play(anim);
    }
  }
